function [t_out, E_out] = removeGDOffset(t_in, E_in, lambda_list, central_lambda, dispersion)
%% Code to remove a constant GD offset from a time-dependent electric field
% WITHOUT modifying the number of elements of the matrices involved (i.e.
% cropping) or adding zero elements (padding).
%
...
%           - central_lambda: Central wavelength for the calculation of the
%               spectral phase.
...
%           - E_out: Same as E_in, but out, so with peak of electric field
%               at time zero.
%
% Date: 07.03.2022
%
...
if dispersion
    if max(abs(E_in(1, :))) > max(abs(E_in(2, :)))
        [~, index_max] = max(abs(E_in(1, :)));
    else
        [~, index_max] = max(abs(E_in(2, :)));
    end
    [~ , spectral_phase]    = getGDCurve(lambda_list, central_lambda, -t_in(index_max));
    [t_out, E_out]          = chirperPlate(t_in, E_in, lambda_list, spectral_phase, 1);
